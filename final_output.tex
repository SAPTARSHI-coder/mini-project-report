...
Classical contemporary engineering approaches predominantly utilize basic exact terminology dictionary matching explicitly against small, rigidly updated static blacklists \cite{schiff1998}. This historically proposed framework uniquely and natively employs a localized ensemble Machine Learning classifier explicitly evaluating an array of 9 deeply diverse sub-string feature metrics definitively representing multiple completely distinct orthographic lexical alignments and vocal phonetic encoding dimensions simultaneously.
...
The integration of Random Forest and Gradient Boosting architectures into this domain allows for the evaluation of non-linear interactions between disparate features \cite{pedregosa2011}. For example, two drugs might share a low Levenshtein distance but possess entirely distinct phonetic Metaphone codes. A simple linear threshold might flag them as confusable, whereas a decision tree model can learn that distinct phonetics often mitigate the risk of visual similarity if the prescription is communicated verbally.
...
The core intelligence of the LASA detection system hinges upon high-quality, real-world data. The dataset explicitly driving the supervised machine learning pipeline is derived comprehensively from the Institute for Safe Medication Practices (ISMP) List of Confused Drug Names \cite{ismp2023}.
...
Primary Source & ISMP Alert Database \cite{ismp2023} \\
